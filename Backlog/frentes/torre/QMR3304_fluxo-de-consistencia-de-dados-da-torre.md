---
id: QMR3304
title: Fluxo de consistência de dados da torre
frente: torre
status: entregue
prioridade: media
fonte: quimera+jira
quimera: 3304
jira: IAF-30
categoria: TI
deliverable_type: Outros
story_points: 5
tipo_origem: Tarefa
responsavel: Leandro Marques Gontijo
criada: 2026-03-17
concluida: 2026-06-05
prazo: null
prazo_estimado: False
rice: null
esforco: null
valor_negocio: null
owner: João Vinícius
dependencias: []
bloqueia: []
tags: [fonte-quimera-jira, cat-ti]
---

# QMR3304 — Fluxo de consistência de dados da torre

## Descrição (espelho — fonte: quimera+jira)

> Origem: Quimera #3304 · Jira IAF-30 · categoria: TI

Fluxo que compara os dados do Hyperflow com os dados da torre para encontrar problemas de consistência

## Rastreabilidade

- **Responsável:** Leandro Marques
- **Quimera:** #3304 (status fonte mapeado → `refinado`)
- **Jira:** IAF-30 (projeto IAF)
- **Categoria fonte:** TI
- **Criada:** 2026-03-17

## Histórico

- 2026-06-02 — Item importado por espelhamento Quimera/Jira (reconciliação de backlog). fonte=quimera+jira.
- 2026-06-15 — Status sincronizado do Quimera #3304: entregue. Concluída 2026-06-05. Responsável: Leandro Marques Gontijo.
