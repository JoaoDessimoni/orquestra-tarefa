---
id: QMR3327
title: Realizar indexação da pesquisa na aba de ações
frente: torre
status: entregue
prioridade: media
fonte: quimera+jira
quimera: 3327
jira: IAF-67
categoria: TI
deliverable_type: Outros
story_points: 5
tipo_origem: Subtarefa
responsavel: Marcos Rodrigues
criada: 2026-04-07
concluida: 2026-06-11
prazo: 2026-06-09
prazo_estimado: True
rice: null
esforco: null
valor_negocio: null
owner: João Vinícius
dependencias: []
bloqueia: [IAF-60]
tags: [fonte-quimera-jira, cat-ti]
---

# QMR3327 — Realizar indexação da pesquisa na aba de ações

## Descrição (espelho — fonte: quimera+jira)

> Origem: Quimera #3327 · Jira IAF-67 · categoria: TI

Implementar ou corrigir a indexação da pesquisa na aba de ações para melhorar a busca e filtragem de informações.

## Rastreabilidade

- **Responsável:** Marcos Rodrigues
- **Quimera:** #3327 (status fonte mapeado → `validacao`)
- **Jira:** IAF-67 (projeto IAF)
- **Categoria fonte:** TI
- **Criada:** 2026-04-07
- **Prazo (estimado):** 2026-06-09
- **Subtarefa de** (bloqueia): IAF-60 — convertida de subtarefa Jira IAF-60

## Histórico

- 2026-06-02 — Item importado por espelhamento Quimera/Jira (reconciliação de backlog). fonte=quimera+jira.
- 2026-06-15 — Status sincronizado do Quimera #3327: entregue. Concluída 2026-06-11. Responsável: Marcos Rodrigues.
